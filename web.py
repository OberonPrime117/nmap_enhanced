...
def custom_nmapfunc(ip, params):
    ...
    command = str(params["Extra"]) +" -oA results/"+str(direct)+"/"+str(direct)+" "+str(ip)
    ...
    if result[0] == None or str(result[0]) == "None":
        ...
        if result[0] == False and result[1]=="ping":
            if os.path.exists("offline_ip_addresses.csv"):
                ...
                df.to_csv('offline_ip_addresses.csv',mode='a',index=False,header=False)
            else:
                data = {'IP Address': [str(ip)]}
                df = pd.DataFrame(data)
            ...
            if os.path.exists("live_ip_addresses.csv"):
                data = {'IP Address': [str(ip)]}
                df = pd.DataFrame(data)
                df.to_csv('live_ip_addresses.csv',mode='a',index=False,header=False)
            else:
                data = {'IP Address': [str(ip)]}
                df = pd.DataFrame(data)
        ...
        if result[0] == True and result[1]=="port":
            ...
            try:
                ...
                nmap_xml_file = os.path.join("results",str(direct),str(xml_direct))
...
def nmapfunc(ip, params):
    ...
    try:
        ...
        pass
    big_statement = str(params["Basic"])+" "+str(params["Technique"])+" "+str(params["Zombie"])+str(params["FTP"])+" "+str(params["UDP"])+" "+str(params["SCTP_INIT"])+" "+str(params["SCTP_COOKIE"])+" "+str(params["HOST_DISCO"])

    if ipaddress.ip_address(str(ip).strip()).version == 4:
        command = "nmap -n "+str(params["Vulners"])+" --open --min-parallelism 10 "+" -sV -Pn "+str(params["Timing"])+" "+str(big_statement)+" "+str(params["PortTechnique"])+" "+str(params["PortText"])+" -oA results/"+str(direct)+"/"+str(direct)+" "+ str(ip)
    if ipaddress.ip_address(str(ip).strip()).version == 6:
        command = "nmap -6 -n "+str(params["Vulners"])+" --open --min-parallelism 10 "+" -sV -Pn "+str(params["Timing"])+" "+str(big_statement)+" "+str(params["PortTechnique"])+" "+str(params["PortText"])+" -oA results/"+str(direct)+"/"+str(direct)+" "+ str(ip)
...
def convert(ip):
    ...
    for filename in files:
        ...
        if os.stat(filename).st_size == 0 or df.empty or np.isnan(df.iloc[0]['PORT']):
            data={ 'IP': [str(ip)], 'PORT' :['-'], 'PROTOCOL': ['-'], 'SERVICE': ['-'], 'VERSION' : ['-'], 'STATE': ['IP is live but ports are closed/filtered'], 'TYPE':['-'], 'CVSS':['-'], 'ID': ['-'], 'IS EXPLOIT AVAILABLE?':['-'],'TIME WHEN SCAN FINISHED':''}
            df = pd.DataFrame(data)
        content.append(df)
...
def csv_xml_switch(nmap_xml_file,csv_output_file):
    ...
    with open(csv_output_file, "w", newline="") as csv_file:
        ...
        writer.writerow(["IP", "PORT", "PROTOCOL", "SERVICE", "VERSION", "STATE",'TYPE','CVSS','ID','IS EXPLOIT AVAILABLE?','TIME WHEN SCAN FINISHED'])
                ...
                if port.find("state").get("state") == "open" or port.find("state").get("state") == "open|filtered":
                    ...
                    try:
                        ...
                        if len(script_tag)>0:
                            for c in script_tag:
                                try:
                                    ...
                                    if val in ["id","cvss","type","is_exploit"]:
                                        if i%4==0 and i!=0:
                                            writer.writerow([ip_address, port.get("portid"), port.get("protocol"), port.find("service").get("name"), str(product) + " " + str(version), port.find("state").get("state"), cve["type"],cve["cvss"],cve["id"],cve["is_exploit"],time])
                                            cve = {}
                                            i = 0
                                        val = c.get("key")
                                        if val == "type":
                            ...
                            writer.writerow([ip_address, port.get("portid"), port.get("protocol"), port.find("service").get("name"), str(product) + " " + str(version), port.find("state").get("state"), '-','-','-','-',time])
                    except:
                        writer.writerow([ip_address, port.get("portid"), port.get("protocol"), port.find("service").get("name"), str(product) + " " + str(version), port.find("state").get("state"), '-','-','-','-',time])
